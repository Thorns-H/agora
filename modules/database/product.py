...
def new_product(name: str, brand: str, description: str, image: str, price: float, category: str, status: str, stock: str, ID_user: int, total_sales: str) -> bool:
    ...
    with agora.cursor() as cursor:
        cursor.execute("INSERT INTO producto(Nombre, Marca, Descripcion, Imagen, Precio, Categoria, Estatus, Existencias, ID_Usuarios, Ventas_Totales) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", 
                       (name, brand, description, image, price, category, status, stock, ID_user, total_sales))
