...
def new_order_products_shipping(ID_cart: int, ID_address: int, issue_date: str, delivery_date: str, status: str, payment: str, shipping_cost: int) -> bool:
    ...
    with agora.cursor() as cursor:
        cursor.execute("INSERT INTO orden_productos_envio(ID_Carrito, ID_Direccion, Fecha_Emision, Fecha_Entrega, Estado, Pago, Precio_Envio) VALUES (%s, %s, %s, %s, %s, %s, %s)", 
                       (ID_cart, ID_address, issue_date, delivery_date, status, payment, shipping_cost))
